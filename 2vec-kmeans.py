...
FP = fm.FontProperties(
    fname='/Users/fujita/Downloads/GenEiGothicAll-1.1/GenEiGothicM/GenEiGothicM-Light.ttf',
    size=7)
...
def clustering(vectors,label_count,learning_count_max=1000):
    ...
    for step in range(learning_count_max):
        ...
        for i, center_vector in enumerate(center_vectors):
            center_vectors[i] = [v/label_vector.count(i) if label_vector.count(i)!=0 else 0 for v in center_vector]
        #各ベクトルのラベルの再割当て
...
# 単語間の類似度計算
#print(model.similarity("ネカマ", "オナベ"))
# 意味の足し引き
#print(model.most_similar(positive=["闇","白"], negative=["黒"]))
...
if __name__=='__main__':
    ...
    for i in range(34):
        if i!= 2:
            axes.annotate(
                        titles[i], xy=(po[i][0], po[i][1]),fontproperties=FP)
