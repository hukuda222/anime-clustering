...
def is_exist(str):
    return str is not None and isinstance(str, type('NoneType'))
...
def ng_block(link):
    ng_list = ["google", "twitter.com", "anipo.jp/anime/",
               "tvanimedouga.blog93.fc2.com/",
               "www.st-trigger.co.jp/", "攻略wiki.", "www.youtube.com"]
    for ng in ng_list:
        if link.find(ng) != -1:
            return False
    return True
...
def get_urls(w, driver):
    ...
    links = []
    for l in all_links:
        if l.prettify().find("href=") != -1 and\
                l.prettify().find("onmousedown") != -1:
